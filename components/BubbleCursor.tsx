...
class Particle {
  ...
  constructor(x: number, y: number) {
    this.initialLifeSpan = Math.floor(Math.random() * 40 + 40); // આયુષ્ય ઘટાડ્યું
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent) => {
      if (e.touches.length > 0) {
        ...
        const dist = Math.hypot(touch.clientX - cursorRef.current.x, touch.clientY - cursorRef.current.y);
        if (dist > 8) { // 8px થી વધુ હલનચલન હોય તો જ બબલ ઉમેરો
    ...
    const onMouseMove = (e: MouseEvent) => {
      const targetX = wrapperElement ? e.clientX - wrapperElement.getBoundingClientRect().left : e.clientX;
      const targetY = wrapperElement ? e.clientY - wrapperElement.getBoundingClientRect().top : e.clientY;

      const dist = Math.hypot(targetX - cursorRef.current.x, targetY - cursorRef.current.y);

      if (dist > 10) { // માઉસ 10px ખસે ત્યારે જ નવું બબલ બનશે
